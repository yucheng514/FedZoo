...
np.random.seed(1)
num_clients = 20
dir_path = str((Path(__file__).resolve().parent / "data" / "EMNIST").resolve()) + "/"


def generate_dataset(dir_path, num_clients, niid, balance, partition, split="byclass"):
...
if __name__ == "__main__":
    ...
    split = sys.argv[4] if len(sys.argv) > 4 else "byclass"

    generate_dataset(dir_path, num_clients, niid, balance, partition, split=split)
